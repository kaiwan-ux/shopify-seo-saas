...
class Blog(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    """Local mirror of a Shopify blog."""

    __tablename__ = "blogs"
    __table_args__ = (UniqueConstraint("store_id", "shopify_id", name="uq_blogs_store_shopify_id"),)
    ...
    articles_count: Mapped[int | None] = mapped_column(nullable=True)
    raw_data: Mapped[dict | None] = mapped_column(JSONB, nullable=True)
    synced_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    is_deleted: Mapped[bool] = mapped_column(default=False, nullable=False)

    store: Mapped["Store"] = relationship("Store", back_populates="blogs")  # noqa: F821

    def __repr__(self) -> str:
        return f"<Blog id={self.id} shopify_id={self.shopify_id} handle={self.handle!r}>"
